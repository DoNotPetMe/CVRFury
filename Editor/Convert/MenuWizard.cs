        ...
        public static string Apply(GameObject avatar, List<Row> rows)
        {
            ...
            foreach (var row in rows.Where(r => r.include))
            {
                ...
                else
                {
                    ...
                    // EVERY toggle ships as a real clip layer that CVRFury builds and attaches itself — the
                    // CCK regenerates the AAS animator and SKIPS any entry whose parameter already exists, so
                    // an entry without its OWN built layer (e.g. gameObjectTargets-only) silently never gets
                    // one and the object is dead. Both states are always explicit: missing OFF is synthesized
                    // (m_IsActive inverts, materials/blendshapes restore to the live scene value).
